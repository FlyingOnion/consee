...
<template>
  <div class="space-y-1 p-2">
    ...
    >
      <div class="p-3" :title="policy.name">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <div
              :class="[
                'w-2 h-2 rounded-full transition-colors duration-200',
                policy.name === current ? 'bg-blue-500' : 'bg-gray-300 group-hover:bg-blue-400',
              ]"
            />
            <div class="min-w-0 flex-1">
              ...
              <p
                :class="[
                  'text-xs truncate',
                  policy.name === current ? 'text-blue-700' : 'text-gray-500',
                ]"
              >
                ID: {{ policy.id }}
              </p>
